...
class ResponseFSC extends Component
{
    use WithPagination;
    ...
    public $clause, $name, $files, $status, $nonconformance, $report_id;
    public $solutions;
    ...
    {
        $this->received = FSCAudits::with('responses', 'creator', 'manager', 'HODs', 'sayings', 'images')->where('id', $id)->first();
    ...
    public function answers()
    {
        ...
        if ($this->report_id) {
            $store = FSCAudits::find($this->report_id);
    ...
    {
        if (auth()->user()->RA) {
            $confomances = FSCAudits::latest()->where('status','pending')
            ...
            $confomances = FSCAudits::where('user_id', '=', auth()->id())->where('status', 'pending')
                ->when($this->search != '', function ($query) {
